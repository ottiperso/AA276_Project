# animate_brt.py
# animated BRT + trajectory videos
# two animations per IC: pz vs vz phase plane, px vs pz position plane
# BRT bg updates each frame: at traj time t, shows BRT at time horizon t
# (so BRT grows forward instead)
...
parser.add_argument('--ic',   type=str, default='inside_brt', help='IC name or "all"')
parser.add_argument('--fps',  type=int, default=30, help='frames per second')
parser.add_argument('--skip', type=int, default=5, help='plot every N trajectory steps')
...
def make_animation(ic_name):
    ...
    pcm1 = ax1.pcolormesh(dpz, dvz, V_s1_all[brt_indices[0]], cmap='RdBu', shading='auto', vmin=-3, vmax=3, alpha=0.85)
    ...
    dot1, = ax1.plot([], [], 'o', color=color, markersize=10, markeredgecolor='k', markeredgewidth=1)
    ...
    pcm2 = ax2.pcolormesh(dpx, dpz, V_s2_all[brt_indices[0]], cmap='RdBu', shading='auto', vmin=-3, vmax=3, alpha=0.85)
    ...
    def update2(i):
        ...
        cf = ax2.contourf(dpx, dpz, V, levels=[V.min(), 0], colors=['red'], alpha=0.15)
