"""
BRT Validation Script
---------------------
Samples random states from the state space, propagates them forward
under optimal (bang-bang) control for both pursuer and evader, and
reports statistics on BRT guarantee violations.

Reports:
  - Trajectories that started INSIDE BRT but never reached failure set (violations)
  - Trajectories that started OUTSIDE BRT but entered failure set (also notable)
  - Trajectories that exited the grid domain
  - Summary statistics

Usage:
  python validate_brt.py --n_samples 500 --t_horizon 10.0 --dt 0.01
"""
...
parser.add_argument('--n_samples',  type=int,   default=500,  help='Number of random initial states')
parser.add_argument('--t_horizon',  type=float, default=10.0, help='Forward simulation time (s)')
...
# sample uniformly from grid domain
z0_samples = np.random.uniform(
    low=GRID_LO, high=GRID_HI,
    size=(args.n_samples, 6)
)

# get V at each sample
...
# ── result containers ──
# for each sample, track outcome
outcomes = []  # dict per sample

# trajectories to save (violations + notable cases)
...
for idx in tqdm(range(args.n_samples), desc='Simulating'):
    ...
    # classify notable trajectories
    traj_arr = np.array(trajectory)

    if started_inside and not captured and not exited_grid:
        # BRT VIOLATION: inside BRT, never captured
    ...
    if not started_inside and captured:
        # surprise capture: outside BRT but got captured
...
with open('outputs/data/validation_summary.json', 'w') as f:
    json.dump(summary, f, indent=2)
print('\nSaved summary to outputs/data/validation_summary.json')

with open('outputs/data/failing_trajectories.json', 'w') as f:
    json.dump(failing_trajectories, f)
print(f'Saved {len(failing_trajectories)} failing trajectories to outputs/data/failing_trajectories.json')
...
ax.hist(inside_cap_V,  bins=30, alpha=0.6, color='green',  label=f'Inside→captured ({len(inside_cap_V)})')
ax.hist(inside_viol_V, bins=30, alpha=0.6, color='red',    label=f'Inside→NOT captured ({len(inside_viol_V)}) VIOLATIONS')
ax.hist(outside_cap_V, bins=30, alpha=0.6, color='orange', label=f'Outside→captured ({len(outside_cap_V)})')
ax.hist(outside_ok_V,  bins=30, alpha=0.6, color='blue',   label=f'Outside→escaped ({len(outside_ok_V)})')
ax.axvline(0, color='k', linestyle='--', linewidth=2, label='BRT boundary (V=0)')
ax.set_xlabel('V(z0)', fontsize=12)
...
if capture_times_inside:
    ...
    ax.axvline(np.mean(capture_times_inside), color='r', linestyle='--',
               label=f'Mean: {np.mean(capture_times_inside):.2f}s')
...
# plot failing trajectories
for ft in failing_trajectories[:50]:  # cap at 50 for readability
    traj = np.array(ft['trajectory'])
    ax.plot(traj[:, 2], traj[:, 5], 'r-', alpha=0.5, linewidth=0.8)
    ax.plot(traj[0, 2], traj[0, 5], 'r.', markersize=5)

# plot a sample of successful captures in green
sample_success = inside_captured[:20]
...
ax.set_title(f'Failing trajectories (red) in phase plane\n({len(failing_trajectories)} violations, {len(inside_captured)} captures)', fontsize=11)
ax.grid(True)

fig.tight_layout()
fig.savefig('outputs/plots/brt_validation.png', bbox_inches='tight')
print('Saved outputs/plots/brt_validation.png')